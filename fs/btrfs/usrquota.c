                               ...
                               struct btrfs_quota_account_rec *oper)
{
	...
	if (sgn == 1) {
		if (usrquota->uq_reserved > oper->num_bytes)
			usrquota->uq_reserved -= oper->num_bytes;
		else
...
int btrfs_usrquota_free_rootid(struct btrfs_fs_info *fs_info, u64 rootid, uid_t uid, u64 num_bytes)
{
	struct btrfs_usrquota *usrquota;
...
int btrfs_usrquota_free(struct btrfs_root *root, uid_t uid, u64 num_bytes)
{
	...
	return btrfs_usrquota_free_rootid(root->fs_info, rootid, uid, num_bytes);
