...
void synoacl_ext4_unregister(void)
{
	EXT4_MODULE = NULL;
}
 ...
 */
static size_t ext4_mod_xattr_list_syno_acl_access(struct inode *inode, char *list, size_t list_len,
			   const char *name, size_t name_len)
{
	if (IS_EXT4_ACL_READY(xattr_list_syno_acl)) {
		return DO_EXT4_ACL(xattr_list_syno_acl, inode, list, list_len, name, name_len);
	}
	return -EOPNOTSUPP;
}

static int ext4_mod_xattr_get_syno_acl_access(struct inode *inode, const char *name,
			  void *buffer, size_t size)
{
	...
	if (IS_EXT4_ACL_READY(xattr_get_syno_acl)) {
		return DO_EXT4_ACL(xattr_get_syno_acl, inode, buffer, size);
	}
	return -EOPNOTSUPP;
}

static int ext4_mod_xattr_set_syno_acl_access(struct inode *inode, const char *name,
			  const void *value, size_t size, int flags)
{
	...
	if (IS_EXT4_ACL_READY(xattr_set_syno_acl)) {
		return DO_EXT4_ACL(xattr_set_syno_acl, inode, value, size);
	}
	return -EOPNOTSUPP;
}
