	...
	void *data)
{
	...
	smp_rmb();
	BUG_ON(atomic_read(&host->cmd_active));
...
static void ox820direct_timeout(unsigned long unused)
{
	...
	host = get_host();
	smp_rmb();
    if (likely(atomic_read(&host->cmd_active))) {
		direct_command_t        *cmd = &host->cmd;
		direct_access_context_t *context = cmd->context;

        printk(KERN_ERR "ox820direct_timeout timeout() cmd %p, context %p %s\n",
			cmd, context, context->read_only ? "reader" : "writer");
        direct_analyse(1);
    } else {
		printk(KERN_ERR "ox820direct_timeout() Spurious interrupt\n");
	}
}
...
static int direct_execute(void)
{
	...
	odrb_dma_sata_sq(cmd->write ? OXNAS_DMA_TO_DEVICE : OXNAS_DMA_FROM_DEVICE,
		cmd->nsects, cmd->dma_list_pa, 0);
...
static irqreturn_t direct_isr(int irq_status, unsigned long arg)
{
    ...
    if (likely(atomic_read(&host->cmd_active))) {
		...
		/* Can't del_timer_sync() in ISR so potential for timer to fire and
		 * be serviced on another CPU just before we disable it below */
		...
		printk(KERN_WARNING "direct_isr() Spurious interrupt\n");
	}
...
static void direct_analyse(int timed_out)
{
	...
	/* The command is entirely finished with */
	atomic_set(&host->cmd_active, 0);
	smp_wmb();
