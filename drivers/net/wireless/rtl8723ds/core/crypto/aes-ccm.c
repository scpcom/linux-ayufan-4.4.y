			       ...
			       u8 *x)
{
	...
	rtc_aes_encrypt(aes, b, x); /* X_1 = E(K, B_0) */
	...
	rtc_aes_encrypt(aes, aad_buf, x); /* X_2 = E(K, X_1 XOR B_1) */

	if (aad_len > AES_BLOCK_SIZE - 2) {
		xor_aes_block(&aad_buf[AES_BLOCK_SIZE], x);
		/* X_3 = E(K, X_2 XOR B_2) */
		rtc_aes_encrypt(aes, &aad_buf[AES_BLOCK_SIZE], x);
	}
}
	       ...
	       const u8 *aad, size_t aad_len, u8 *crypt, u8 *auth)
{
	...
	aes = rtc_aes_encrypt_init(key, key_len);
	if (aes == NULL)
		return -1;
