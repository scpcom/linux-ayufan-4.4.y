...
#ifdef MY_DEF_HERE
#define DISK_READY_MAX_WAIT_MSEC 120000 // 120s
...
#ifdef MY_DEF_HERE
	/*
	 * Initialize the syno_drive_start_time here to prevent the timeout
	 * mechanism becoming invalid (jiffies - 0 must be greater than 120s)
	 * from that there's error on low level driver while setting syno_drive_start_time.
	 */
	atomic_long_set(&syno_drive_start_time, jiffies);
