...
/** pfe_tso
 */
int pfe_common_tso( struct sk_buff *skb, struct hif_client_s *client, struct tso_cb_s *tso, int queuenum, u32 init_ctrl)
{
	struct skb_shared_info *sh = skb_shinfo(skb);
	skb_frag_t *f=NULL;
	struct tcphdr *th;
	...
	unsigned int f_id=0;	/* id of the current fragment */
	...
	n_desc = pfe_common_tso_to_desc(skb);
	 ...
	 *                                  sh->gso_segs (hif/tso segment header)
         */
	if (unlikely((__hif_tx_avail(&pfe->hif) < (n_desc + 2 * sh->gso_segs))||
			(hif_lib_tx_avail(client, queuenum) <  (n_desc + 2 * sh->gso_segs)))) {
		//netif_err(priv, tx_err, priv->dev, "%s: not enough descriptors:%d %d %d\n",
		//		__func__,  __hif_tx_avail(&pfe->hif), hif_lib_tx_avail(client, queuenum), n_desc + 2 * sh->gso_segs);
		kfree_skb(skb);
		/*FIXME increment stats in client based on return value */
...
#ifdef PFE_TSO_STATS
	tso->tso_len_counters[((u32)skb->len >> 11) & 0x1f]++;
#endif
	 ...
	 */
	wrIndx = hif_lib_get_tx_wrIndex(client, queuenum);
	q_dma_base = (struct hif_frag_dma_map_s *)&tso->dma_map_array[queuenum][0];
...
//		netif_info(priv, tx_queued, priv->dev, "%s: seq: %x, id: %x, ctrl: %x\n", __func__, seq, id, ctrl);
		...
		/* hif/tso header */
		__hif_lib_xmit_tso_hdr(client, queuenum, ctrl, ip_off, id, ip_tcp_hdr_len + s_len, tcp_off, seq);

		id++;
		seq += s_len;

		/* ethernet/ip/tcp header */
		__hif_lib_xmit_pkt(client, queuenum, (void *)f_dma_first->data, sh_len, 0, HIF_DONT_DMA_MAP, skb);
		...
		while (l_len && s_len)
		{
			...
			__hif_lib_xmit_pkt(client, queuenum,  (void *)((u32)f_dma->data + off),
		...
		while (s_len) {
			...
			__hif_lib_xmit_pkt(client, queuenum, (void *)((u32)f_dma->data + f_off), len, 0, hif_flags, skb);
