...
static int at803x_set_wol(struct net_device *ndev,
...
static void at803x_get_wol(struct net_device *ndev,
                        struct ethtool_wolinfo *wol)
{
    u32 value;
    struct phy_device *phydev;
...
try_again:
	...
	|| (hif_lib_tx_credit_avail(pfe, priv->id, queuenum) < n_segs))) {
		/* FIX BUG-106417 */
		dbg_print_info("%s : __hif_tx_avail(&pfe->hif):%u, hif_lib_tx_avail(&priv->client, queuenum):%u, hif_lib_tx_credit_avail(pfe, priv->id, queuenum):%u, n_desc:%u, n_segs:%u, queuenum:%d\n",
			__func__, __hif_tx_avail(&pfe->hif), hif_lib_tx_avail(&priv->client, queuenum),
			hif_lib_tx_credit_avail(pfe, priv->id, queuenum), n_desc, n_segs, queuenum);
...
static void pfe_tso(struct sk_buff *skb, struct pfe_eth_priv_s *priv, int queuenum)
{
	...
	netif_info(priv, tx_queued, priv->dev,"%s: orig_seq: %x, orig_id: %x, seg_size: %d, seg_n: %d, data_len: %d, hdr_len: %d skb_fras: %d\n",
					__func__, seq, id, sh->gso_size, sh->gso_segs, data_len, sh_len, sh->nr_frags);
...
//Patch by QNAP:
	...
	sprintf(bus_id, "comcerto-%d", priv->einfo->bus_id);
	snprintf(phy_id, MII_BUS_ID_SIZE + 3, PHY_ID_FMT, bus_id, priv->einfo->phy_id);
	if(bus_find_device_by_name(&mdio_bus_type, NULL, phy_id) == NULL)
		return -ENODEV;
	else
////
		return 0;
