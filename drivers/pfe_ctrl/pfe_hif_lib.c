...
static int hif_lib_client_init_rx_buffers(struct hif_client_s *client, int q_size)
{
	...
	client->rx_qbase = kzalloc(client->rx_qn * q_size * sizeof(struct rx_queue_desc), GFP_KERNEL & ~__GFP_WAIT);
	if (!client->rx_qbase){
		goto err;
	}
...
static int hif_lib_client_init_tx_buffers(struct hif_client_s *client, int q_size)
{
	...
	client->tx_qbase = kzalloc(client->tx_qn * q_size * sizeof(struct tx_queue_desc), GFP_KERNEL & ~__GFP_WAIT);
	if (!client->tx_qbase) {
		return 1;
	}
