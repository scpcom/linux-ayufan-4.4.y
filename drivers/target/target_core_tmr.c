	...
	struct se_session *se_sess)
{
	...
	list_for_each_entry_safe(se_cmd, tmp_cmd,
		...
		|| se_cmd->t_state == TRANSPORT_PROCESS_WRITE
			...
			pr_err("oop!!! t_state:0x%x\n", se_cmd->t_state);
		}

		/* Now, the status may be 
		 *
		 * (1) TRANSPORT_COMPLETE but CMD_T_SEND_STATUS not set
		 * (2) or, keep TRANSPORT_PROCESSING (transport_complete_task()
		 *     doesn't be called yet)
		 */
	...
	struct list_head *preempt_and_abort_list)
{
	...
	list_for_each_entry_safe(cmd, tcmd, &qobj->qobj_list, se_queue_node) {
		...
		pr_debug("LUN_RESET[drain cmd]: found cmd(ITT:0x%8x)\n", 
			cmd->se_tfo->get_task_tag(cmd));
