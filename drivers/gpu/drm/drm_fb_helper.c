			    ...
			    struct fb_info *info)
{
	 ...
	 */
	if (var->bits_per_pixel > drm_format_info_plane_bpp(fb->format, 0) ||
		...
		DRM_DEBUG("fb requested width/height/bpp can't fit in current fb "
			  ...
			  fb->width, fb->height, drm_format_info_plane_bpp(fb->format, 0));
		return -EINVAL;
	}
	 ...
	 */
	var->bits_per_pixel = drm_format_info_plane_bpp(fb->format, 0);
				   ...
				   uint32_t fb_width, uint32_t fb_height)
{
	...
	info->var.bits_per_pixel = drm_format_info_plane_bpp(fb->format, 0);
