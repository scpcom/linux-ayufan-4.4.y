...
_mali_osk_errcode_t _mali_ukk_gp_start_job_v800(void *ctx,
		_mali_uk_gp_start_job_v800_s *uargs)
{
	struct mali_session_data *session;
	struct mali_gp_job *job;
	mali_timeline_point point;
	...
	job = mali_gp_job_create_v800(session, uargs, mali_scheduler_get_new_id(),
				 NULL);
	if (NULL == job) {
		MALI_PRINT_ERROR(("Failed to create GP job.\n"));
		return _MALI_OSK_ERR_NOMEM;
		...
		_mali_uk_pp_and_gp_start_job_s *uargs)
{
	...
	_mali_uk_gp_start_job_v800_s __user *gp_args_v800;
	...
	gp_args_v800 = (_mali_uk_gp_start_job_v800_s __user *)(uintptr_t)kargs.gp_args;
	...
	if (session->version == _MALI_UK_API_VERSION_V800)
		gp_job = mali_gp_job_create_v800(session, gp_args_v800,
				    mali_scheduler_get_new_id(),
